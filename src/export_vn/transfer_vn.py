...
class Jobs:
    ...
    def add_job_once(self, job_fn, args=None, kwargs=None):
        job_name = args[0] if args is not None and len(args) > 0 else "job"
...
#         logger.exception(_("Exception in subprocess"))
...
#     jobs_o = Jobs(
#         url="sqlite:///" + settings["TUNING"]["sched_sqllite_file"],
#         nb_executors=settings["TUNING"]["sched_executors"],
#     )
#     with jobs_o as jobs:
...
#             jobs.add_job_once(job_fn=full_download_1, args=["entities", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["FAMILIES"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["families", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["FIELDS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["fields", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["LOCAL_ADMIN_UNITS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["local_admin_units", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["OBSERVATIONS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["observations", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["OBSERVERS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["observers", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["PLACES"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["places", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["SPECIES"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["species", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["TAXO_GROUPS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["taxo_groups", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["TERRITORIAL_UNITS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["territorial_units", settings.as_dict()])  # type: ignore
#         if settings["CONTROLER"]["VALIDATIONS"]["enabled"]:
#             jobs.add_job_once(job_fn=full_download_1, args=["validations", settings.as_dict()])  # type: ignore
