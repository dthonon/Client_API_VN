...
class DownloadVn:
    ...
    def store(self, opt_params_iter=None):
        ...
        for opt_params in opt_params_iter:
            i += 1
            logger.debug(_('Iteration %s, opt_params = %s'),
                          i, opt_params)
...
class Observations(DownloadVn):
    ...
    def _store_list(self, id_taxo_group, by_specie, short_version='1'):
        ...
        """
        # GET from API
        logger.debug(_('Getting observations from controler %s, using API list'),
                      self._api_instance.controler)
        ...
        for taxo in taxo_groups:
            if taxo['access_mode'] != 'none':
                ...
                logger.info(_('Getting observations from taxo_group %s, in _store_list'),
                             id_taxo_group)
                if by_specie:
                    ...
                    for specie in species:
                        if specie['is_used'] == '1':
                            logger.info(_('Getting observations from taxo_group %s, species %s'),
            ...
            if taxo['access_mode'] != 'none':
                ...
                while start_date > min_date:
                    ...
                    nb_obs = self._backend.store(self._api_instance.controler,
                                                 ...
                                                 items_dict)
                    logger.info(_('Iter: %s, %s obs, taxo_group: %s, date: %s, interval: %s'),
    ...
    def _list_taxo_groups(self, id_taxo_group):
        ...
        if id_taxo_group == None:
            ...
            for taxo in taxo_groups['data']:
                if taxo['access_mode'] != 'none':
                    logger.debug(_('Starting to download observations from taxo_group %s: %s'),
                                  taxo['id'], taxo['name'])
            ...
            if since is not None:
                ...
                for item in items_dict:
                    ...
                    if item['modification_type'] == 'updated':
                        ...
                        logger.error(_('Observation %s has unknown processing %s'),
